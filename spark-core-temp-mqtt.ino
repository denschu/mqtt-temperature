...
float temp;
double publishedTemp; //make current temperature available to Spark Cloud calls

byte ip[] = { 54, 186, 227, 52 };

TCPClient tcpClient;
PubSubClient client(ip, 1883, 0, tcpClient);

char resultstr[64];
 
void setup() {
  Serial.begin(9600);
  Spark.variable("temp", &publishedTemp, DOUBLE);
...
void loop() {
  ...
  if (client.connect("spark")) {
      sprintf(resultstr, "{\"tmp\":%.2f}", temp); 
      Serial.println("Send data to topic spark");
      client.publish("spark",resultstr);
      Serial.println("success! and try to disconnect");
      client.disconnect();
